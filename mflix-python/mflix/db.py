...
def logout_user(email):
    ...
    try:
        ...
        result = db.sessions.delete_one({ "user_id": email })
        return {"success": result.acknowledged}
    except Exception as e:
        return {"error": e}
...
def delete_user(email):
    ...
    try:
        ...
        sessions_result = db.sessions.delete_one({ "user_id": email })
        users_result = db.users.delete_one({ "email": email })
...
def most_active_commenters():
    ...
    rc = ReadConcern('majority') #db.comments.read_concern # you may want to change this read concern!
